...
from app.infrastructure.models import Bid, EscrowTransaction, Home, Job, Message, MessagePhoto, Photo, Room, User
...
class JobService:
    ...
    async def award_job(self, job_id: int, bid_id: int, user: dict[str, Any]) -> dict[str, Any]:
        ...
        if settings.hedera_is_real:
            if not supplier.hedera_public_key:
                raise HTTPException(status_code=400, detail="supplier_missing_hedera_key")
            escrow_svc = EscrowService()
            escrow_id = escrow_svc.create_escrow_account_with_public_keys(supplier.hedera_public_key)
            job.escrow_account_id = escrow_id
        else:
            escrow_id = f"0.0.{99990 + job_id}"
            job.escrow_account_id = escrow_id
        ...
        await self.insert_message(
            job_id, None, "system",
            f"Escrow account {escrow_id} created. Awaiting funding of {bid.amount_tinybar} HBAR.",
            [],
        )
        return {
            "job_id": job_id,
    ...
    async def fund_job(self, job_id: int, user: dict[str, Any], transaction_id: str | None = None) -> dict[str, Any]:
        job_result = await self.session.execute(select(Job).where(Job.id == job_id, Job.owner_user_id == user["id"]))
        job = job_result.scalar_one_or_none()
        if job is None:
            raise HTTPException(status_code=404, detail="not_found")
        ...
        if transaction_id:
            ...
            if confirmed:
                ...
                tx = EscrowTransaction(
                    ...
                    token="HBAR",
                    status="settled",
                    hedera_tx_id=transaction_id,
                    created_at=now,
                )
                self.session.add(tx)
                balance = escrow_svc.get_balance(escrow_id)
                job.status = "funded"
                job.updated_at = now
                await self.add_audit(self.session, job_id, "escrow_funded", {"amount": bid.amount_tinybar, "tx_id": transaction_id})
                await self.insert_message(job_id, None, "system", f"Funding confirmed! {bid.amount_tinybar} HBAR in escrow.", [])
                asyncio.create_task(self._trigger_assistant_lazy(job_id, user["id"], "Funding confirmed for job", user.get("user_type", "human")))
                return {"job_id": job_id, "status": "funded", "escrow_account_id": escrow_id, "amount_tinybar": bid.amount_tinybar, "balance": balance, "tx_hash": transaction_id}
            else:
                return {"job_id": job_id, "status": "funding_timeout", "escrow_account_id": escrow_id, "amount_tinybar": bid.amount_tinybar}

        balance = escrow_svc.get_balance(escrow_id)
        if balance >= bid.amount_tinybar:
            job.status = "funded"
            job.updated_at = self.now_iso()
            await self.add_audit(self.session, job_id, "escrow_funded", {"amount": bid.amount_tinybar})
            await self.insert_message(job_id, None, "system", f"Funding confirmed! {bid.amount_tinybar} HBAR in escrow.", [])
            return {"job_id": job_id, "status": "funded", "escrow_account_id": escrow_id, "amount_tinybar": bid.amount_tinybar, "balance": balance}

        return {"job_id": job_id, "status": "awaiting_funding", "escrow_account_id": escrow_id, "amount_tinybar": bid.amount_tinybar}
    ...
    async def confirm_job(self, job_id: int, user: dict[str, Any]) -> dict[str, Any]:
        ...
        supplier = await get_user(self.session, job.supplier_user_id)
        if not supplier.hedera_account_id:
            raise HTTPException(status_code=400, detail="supplier_missing_hedera_account")

        escrow_svc = EscrowService()
        tx_id = escrow_svc.release_escrow(escrow_id, supplier.hedera_account_id, bid.amount_tinybar)
        ...
        job.status = "completed"
        job.updated_at = now
        await self.add_audit(self.session, job_id, "job_completed", tx_id)
        return {"job_id": job_id, "status": "completed", "tx_hash": tx_id}
    ...
    async def create_message(self, job_id: int, body: str, photo_ids: list[int], user: dict[str, Any]) -> dict[str, Any]:
        ...
        if body.strip():
            asyncio.create_task(self._trigger_assistant_lazy(job_id, user["id"], body, user.get("user_type", "human")))
        return {
            "id": msg.id,
    ...
    async def _trigger_assistant_lazy(self, job_id: int, user_id: int, body: str, user_type: str) -> None:
        try:
            from agent.service import trigger_assistant
            await trigger_assistant(job_id, user_id, body, user_type)
        except Exception:
            import logging
            logging.getLogger("escroweye.agent").exception("trigger_assistant failed for job %s", job_id)
