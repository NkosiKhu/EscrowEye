...
async def add_audit(
    ...
    seq_result = await session.execute(select(func.coalesce(func.max(AuditEvent.sequence_number), 0) + 1).where(AuditEvent.job_id == job_id))
...
async def audit_events(session: AsyncSession, job_id: int) -> dict[str, Any]:
    ...
    result = await session.execute(select(AuditEvent).where(AuditEvent.job_id == job_id).order_by(AuditEvent.sequence_number))
