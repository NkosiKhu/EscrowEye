...
def create_ai_validation_router(*, db: Callable, now_iso: Callable[[], str], current_user: Callable) -> APIRouter:
    router = APIRouter(prefix="/api", tags=["ai-validation"])

    @router.post("/service-requests/{request_id}/ai-validation/run")
    async def run_service_ai_validation(request_id: int, user: dict[str, Any] = Depends(current_user)):
        _ = user
    ...
    async def get_service_ai_validation(request_id: int, user: dict[str, Any] = Depends(current_user)):
        _ = user
        async with db() as session:
    ...
    async def request_ai_corrections(request_id: int, body: dict[str, Any], user: dict[str, Any] = Depends(current_user)):
        _ = user
        async with db() as session:
