...
async def _trigger_review_lazy(job_id: int) -> None:
    try:
        from agent.service import trigger_review
        await trigger_review(job_id)
    except Exception:
        import logging
        logging.getLogger("escroweye.agent").exception("trigger_review failed for job %s", job_id)
...
def create_jobs_router(
    *,
    ...
    async def fund_job(job_id: int, body: FundJobIn = FundJobIn(), user: dict[str, Any] = Depends(current_user)):
        async with db() as session:
            return await service(session).fund_job(job_id, user, body.transaction_id)
    ...
    async def upload_photos(
        ...
        asyncio.create_task(_trigger_review_lazy(job_id))
